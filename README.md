...
- Node.js `18+`
- 包管理建议：`uv`
- Docker / Docker Compose，用于启动 Redis、PostgreSQL/pgvector；Milvus、etcd、MinIO 作为可选向量库依赖
...
PGVECTOR_TABLE_NAME=wei_quiz_vectors
PGVECTOR_EMBED_DIM=1536

# Optional Milvus backend. Set VECTOR_STORE_BACKEND=milvus to use it.
...
```bash
set VECTOR_STORE_BACKEND=milvus
...
- PostgreSQL：`5432`
- Redis：`6379`
- Milvus：`19530`，仅在 `--profile milvus` 启动时需要
- Milvus health：`9091`，仅在 `--profile milvus` 启动时需要
- MinIO API：`9000`
- MinIO Console：`9001`
...
- FastAPI 后端 + Vue 3 前端 + pgvector 默认向量库，Milvus 可选切换。
...
- Leaf-only 向量化：只将 leaf chunk 写入向量库，parent/root 存入 PostgreSQL。
- Hybrid Retrieval：pgvector/Milvus 向量检索 + BM25 + RRF 融合。
...
6. 删除文档：清理向量库和 PostgreSQL parent store 中的旧节点。
...
app/
  ...
  config.py                      全局配置
  rag_milvus.py                  LlamaIndex + pgvector/Milvus + BM25 + RRF + Rerank
...
- 向量与检索：pgvector 默认向量库、Milvus 可选后端、BM25、QueryFusionRetriever、RRF、DashScope Rerank。
